...
var StdIn = React.createClass({displayName: "StdIn",
  getInitialState: function() {
    return {typedInput: '', complete: false};
  },
  type: function() {
    if(this.state.typedInput != this.props.input) {
      var length = this.state.typedInput.length + 1
      var typedInput = this.props.input.slice(0, length)
      this.setState({typedInput: typedInput, complete: false});
    } else if(!this.state.complete) {
      this.setState({typedInput: this.props.typedInput, complete: true});
    }
  },
    ...
    return (
      React.createElement("div", {className: "shell"}, 
        React.createElement(Command, {
          stdin: React.createElement(StdIn, {input: "whoami"}), 
          stdout: React.createElement(StdOut, {output: "guest"})}
        ), 
