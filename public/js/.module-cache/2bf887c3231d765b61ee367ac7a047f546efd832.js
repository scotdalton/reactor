...
var COMMANDS = [
  ...
  React.createElement(Command, {
    onComplete: this.runNextCommand, 
   input: "whereami", 
   output: React.createElement(StdOut, {output: "the website of Scot Dalton"})}
  ),
  React.createElement(Command, {
    onComplete: this.runNextCommand, 
    input: "pwd", 
    output: React.createElement(StdOut, {output: React.createElement(Directory, {name: "/home/scot"})})}
  ),
  React.createElement(Command, {
    onComplete: this.runNextCommand, 
    input: "ls -a", 
    output: React.createElement(Ls, {files: FILES})}
  ),
  React.createElement(StdIn, {input: "cat README"})
]

var Shell = React.createClass({displayName: "Shell",
  commands: function() {this.props.commands.map(
    function(command) {
      console.log(command.props.input)
    }
  )},
  getInitialState: function() {
    return {commands: []};
  ...
  runNextCommand: function() {
    if(this.state.commands.length != this.commands().length) {
      var length = this.state.commands.length + 1;
      this.setState({commands: this.commands.slice(0, length)});
    }
  },
  componentDidMount: function() {
    this.runNextCommand();
